...
public class PlayerDisplayNameManage {
    ...
    public static String upView(Player player,DataCon dc) {
        ...
        if (!suff.isEmpty()) sb.append(suff);
    ...
    public static BaseComponent[] getDisplayName(Player player,DataCon dc) {
        ...
        cb.event(new HoverEvent(HoverEvent.Action.SHOW_TEXT,new ComponentBuilder(player.getDisplayName() + (dc.getBoolean("Authenticate.Success") ? "\n§b§l已正版认证√" : "") + "\n§f点我发起私聊").create()));
        cb.event(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND,"/tell " + player.getName() + " "));
        //伴侣
        final String mar = MoeInfo.plugin.externalManage.marriageMaster == null ? null : MoeInfo.plugin.externalManage.marriageMaster.DB.GetPartner(player.getName());
        if (mar != null){
            cb.append("§l§c❤");
            cb.event(new HoverEvent(HoverEvent.Action.SHOW_TEXT,new ComponentBuilder("§b伴侣: §f\n" + mar).create()));
        }
        return cb.create();
    }
}
